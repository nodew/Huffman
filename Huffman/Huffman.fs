...
type FreqTable<'a when 'a: comparison> = Map<'a, int>
...
type Weighted<'a> =
    ...
    override x.ToString() =
        match x with
        | Weighted (w, a) -> sprintf "(%d, %s)" w (a.ToString())
...
type WeightedHuffumanTree<'a when 'a: comparison> = Weighted<HuffmanTree<'a>>
...
module HuffmanTree =
    ...
    let mergeWHT (Weighted (w1, htA)) (Weighted (w2, htB)) = Weighted(w1 + w2, merge htA htB)

    let private genPriorityQueue (freqTable: FreqTable<'a>) =
        let folder pQueue (x, w) =
            PriorityQueue.insert (Weighted(w, x)) pQueue

        Map.toSeq freqTable
        |> Seq.fold folder PriorityQueue.empty
    ...
    let fromPriorityQueue pQueue =
        let transform item =
            match item with
            | Weighted (w, a) -> Weighted(w, from a)
...
module HuffmanCodeTable =
    ...
    let lookup x (m: Map<'a, HuffmanCode>) = Map.tryFind x m

    let getSize (freqTable: FreqTable<'a>) (codeTable: HuffmanCodeTable<'a>) =
        codeTable
        |> Map.fold (fun sum key encoding ->
            uint64
                (Map.find key freqTable
                 * sizeof<'a>
                 * encoding.Length)
            + sum) (uint64 (0))
